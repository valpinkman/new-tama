...
export const getLastPost = async () => {
  const fetched = await wp.posts().perPage(1).embed();
  return fetched;
};

export const getSinglePostBySlug = async (slug: string) => {
  const fetched = await wp.posts().slug(slug).embed();
  return fetched;
};

export const getSinglePostById = async (id: number) =>
  wp.posts().id(id).embed();
...
export const getAuthor = (authors: Array<{ id: string }>, authorId: string) =>
  authors.find(getById(authorId));

export const getFeaturedMedia = (
  featuredMedia: Array<{ source_url: string }> = [],
